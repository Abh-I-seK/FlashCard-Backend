    ...
    if(token){
        jwt.verify(token, process.env.JWT_SECRET as string, async(err, decoded) => {
            if(err){
                ...
                const user = await db.select().from(Admin).where(eq(Admin.adminName,decoded as string));
                if(user[0]){
                    next();
                }else{
                    res.send({success:false , msg : "token invalid"});
...
app.post('/signup',async(req , res)=>{
    ...
    if(admin[0]){
        ...
        const token = jwt.sign(adminName, process.env.JWT_SECRET as string, { algorithm: 'HS256' });
